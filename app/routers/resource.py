...
@resource_router.post("/groups/{group_id}/resources", response_model=schemas.ResourceResponse)
...
@resource_router.get("/groups/{group_id}/resources", response_model=list[schemas.ResourceResponse])
...
@resource_router.patch("/resources/{resource_id}", response_model=schemas.ResourceResponse)
async def update_resource(
    ...
    if current_user.role not in [models.UserRole.ADMIN, models.UserRole.TEACHER] and resource.user_id != current_user.id:
    ...
    if file is not None:
        if file.filename:
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Could not save new file: {str(e)}")
            ...
            raise HTTPException(status_code=500, detail=f"{file.filename} is an invalid filename")
...
@resource_router.delete("/resources/{resource_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_resource(
    resource_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
