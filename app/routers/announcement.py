...
@announcement_router.get(
    "/groups/{group_id}/announcement", response_model=List[schemas.Announcement]
)
def get_announcements(
    ...
    if current_user.role != models.UserRole.ADMIN:
        if group_id not in current_group_ids:
            raise HTTPException(
                status_code=403,
                detail="You can't access announcements from a group you're not part of",
            )

    group_exist = (
        db.query(Group)
        .filter(models.User.groups.any(models.Group.id.in_(current_group_ids)))
        .first()
    )
...
@announcement_router.post(
    "/groups/{group_id}/announcement", response_model=schemas.Announcement
)
def create_announcements(
    ...
    if current_user.role not in [
        ...
        raise HTTPException(
            status_code=403,
            detail="You are not allowed to create annoucements in this group",
        )

    if group_id not in current_group_ids and current_user.role != models.UserRole.ADMIN:
        raise HTTPException(
            status_code=403,
            detail="You can't create announcements in a group you're not part of",
        )

    if new_annoucement_data.title.strip() == "":
        raise HTTPException(
            status_code=422, detail="You can't create announcements with an empty title"
        )
...
def delete_announcement(
    ...
    if not annoucement:
        raise HTTPException(
            status_code=404, detail=f"Announcement {announcement_id} not found"
        )

    current_group_ids = [g.id for g in current_user.groups]

    if (
        annoucement.group_id not in current_group_ids
        and current_user.role != models.UserRole.ADMIN
    ):
        raise HTTPException(
            status_code=403,
            detail="You can't delete an announcement in a group you're not part of",
        )
    ...
    if (
        ...
        and current_user.id != annoucement.user_id
    ):
        raise HTTPException(
            status_code=403,
            detail="You are not allowed to delete annoucements in this group",
        )
