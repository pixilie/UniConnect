...
def vote_for_choice(
    ...
    choice = db.query(models.Choice).filter(models.Choice.id == choice_id, models.Choice.poll_id == poll_id).first()
    if not choice:
    ...
    new_vote = models.Vote(user_id=current_user.id, poll_id=poll_id, choice_id=choice_id)
    db.add(new_vote)
    db.commit()
