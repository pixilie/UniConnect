...
function renderStudent(member) {
    ...
    if (member.role === "delegate") {
        ...
        badge.style.cssText = "background:#EADDFF; padding: 2px 6px; border-radius:4px; margin-left:8px; color:#21005D;";
...
document.getElementById('confirmStartElectionBtn').addEventListener('click', async () => {
    ...
    const optionsArray = optionsRaw.split(',').map(opt => opt.trim()).filter(opt => opt.length > 0);
