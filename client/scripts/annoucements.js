...
async function LoadAnnoucements() {
    ...
    if (!res.ok) {
        ...
        data.forEach(element => {
            let first_name = element.creator.first_name;
            let last_name = element.creator.last_name;
            //let role = element.creator.role;
            let title=element.title;
            let content=element.content;
            let urgent=element.urgent;
            addAnnoucement(first_name,last_name,role,title,content,data,urgent);
        });
    }
}

function addAnnoucement(first_name,last_name,role,title,content,date,urgent){
    const annoucementsNode = annoucementTemplate.content.cloneNode(true);
    ...
    displayBox.scrollTop = chat.scrollHeight;
}
